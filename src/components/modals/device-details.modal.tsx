...
const DeviceDetailsModal = ({ show, setShow, deviceDetails }: { show: boolean, setShow: (show: boolean) => void, deviceDetails: KnownDevice }) => {

  return (
    <Dialog open={show} onOpenChange={setShow}>
      <DialogContent>
        <DialogTitle>{deviceDetails.DeviceName.split(" ")[0]}{deviceDetails.isCurrentDevice ? " (Current)" : ""}</DialogTitle>
        <DialogDescription className="sr-only">
          {deviceDetails.DeviceName.split(" ")[1] && deviceDetails.DeviceName.split(" ")[1]}
        </DialogDescription>
        <div className="grid">
          <div>
            <div className="font-bold">Device Name</div>
            <div className="text-muted-foreground">{deviceDetails.DeviceName}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-muted-foreground">Near {deviceDetails?.City} {deviceDetails?.Region}, {deviceDetails?.Country}</div>
          </div>
        </div>
      </DialogContent>
    </Dialog>
