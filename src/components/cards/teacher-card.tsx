      ...
      <Card>
        <CardHeader>
          <div className="flex flex-row items-center gap-4">
            <ProfilePhoto src={user.avatar || ''} givenName={user.givenName || ''} familyName={user.familyName || ''} email={user.email || ''} />
            <div>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <ul>{user.teachableCourses?.map((course) => (
              <li key={course._id}>
                <div>{course.className}</div>
                <div className="text-sm text-muted-foreground">{course?.grades?.map((grade) => gradeLabel(Number(grade))).join(', ')}</div>
              </li>
            ))}</ul>
          </div>
        </CardContent>
