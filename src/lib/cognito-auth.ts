...
function tokensFromSession(session: Awaited<ReturnType<typeof fetchAuthSession>>): {
  AccessToken?: string;
  IdToken?: string;
  RefreshToken?: string;
} {
  ...
  const toJwt = (x: unknown): string | undefined => {
    ...
    if (x && typeof (x as { toString?: () => string }).toString === "function") return (x as { toString: () => string }).toString();
    return undefined;
  };
  return {
...
export type SignInWithCognitoResult =
  | { success: true; tokens: { AccessToken?: string; IdToken?: string; RefreshToken?: string } }
  | {
      success: false;
      challenge: true;
...
export async function signInWithCognito(
  ...
  try {
    ...
    const totpSetupUri =
      "totpSetupDetails" in nextStep && typeof (nextStep as { totpSetupDetails?: { getSetupUri?: () => { href: string } } }).totpSetupDetails?.getSetupUri === "function"
        ? (nextStep as { totpSetupDetails: { getSetupUri: () => { href: string } } }).totpSetupDetails.getSetupUri().href
        : undefined;
 ...
 */
export async function confirmSignInAndGetTokens(challengeResponse: string): Promise<{
  AccessToken?: string;
  IdToken?: string;
  RefreshToken?: string;
}> {
